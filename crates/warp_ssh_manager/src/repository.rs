...
use crate::types::{AuthType, NodeKind, ResolvedSshAuth, SshNode, SshServerInfo};
...
impl SshRepository {
    ...
    pub fn resolve_server_auth(
        ...
        match server.auth_type {
            ...
            AuthType::OneKey => Err(SshRepositoryError::NotFound("onekey credential".to_string())),
        }
    }
...
pub(crate) fn setup_in_memory() -> SqliteConnection {
    ...
    for up in [
        ...
        // 创建 credential_id 列（ssh_onekey_credentials 表随后被下面 drop）
        include_str!(
            "../../persistence/migrations/2026-06-08-120000_add_ssh_onekey_credentials/up.sql"
        ),
        include_str!(
            "../../persistence/migrations/2026-07-31-000000_onekey_credentials_and_drop_ssh_onekey/up.sql"
        ),
    ] {
