...
use crate::{
    ...
    search::{
        ...
        onekey::{
            data_source::OneKeyDataSource,
            searcher::{OneKeySearchItemAction, OneKeySearchMixer},
        },
...
enum PanelMode {
    Searching,
    SendModeSelection {
        credential: warp_onekey::OneKeyCredential,
    },
}

pub struct OneKeyPanel {
    scroll_state: ScrollStateHandle,
    list_state: UniformListState,
    search_bar: ViewHandle<SearchBar<OneKeySearchItemAction>>,
    search_bar_state: ModelHandle<SearchBarState<OneKeySearchItemAction>>,
    mixer: ModelHandle<OneKeySearchMixer>,
    handle: WeakViewHandle<Self>,
    mode: PanelMode,
}

use warp_onekey::SendMode;

#[derive(Clone, Debug)]
pub enum OneKeyPanelAction {
    ResultClicked {
        result_index: usize,
        result_action: Box<OneKeySearchItemAction>,
    },
    SendPasswordOnly(warp_onekey::OneKeyCredential),
    SendUsernameThenPassword(warp_onekey::OneKeyCredential),
    Close,
}

pub enum OneKeyPanelEvent {
    ItemSelected {
        credential: warp_onekey::OneKeyCredential,
        mode: SendMode,
    },
...
impl OneKeyPanel {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let mixer = ctx.add_model(|_| OneKeySearchMixer::new());
                    ...
                    QueryResultRenderer::new(
                        ...
                        |result_index, result_action, event_ctx| {
                            event_ctx.dispatch_typed_action(
                                OneKeyPanelAction::ResultClicked {
                                    result_index,
    ...
    pub fn setup(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.mixer.update(ctx, |mixer, ctx| {
            mixer.reset(ctx);
            if let Ok(data_source) = OneKeyDataSource::new() {
        ...
        ctx.emit(OneKeyPanelEvent::Open);
    }

    pub fn close(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.emit(OneKeyPanelEvent::Close);
    }

    fn handle_search_bar_event(
        &mut self,
        event: &SearchBarEvent<OneKeySearchItemAction>,
    ...
    fn handle_result_selected(
        &mut self,
        result_action: OneKeySearchItemAction,
        ctx: &mut ViewContext<Self>,
    ) {
        match result_action {
            OneKeySearchItemAction::SelectCredential(credential) => {
    ...
    fn handle_send_password_only(
        ...
        ctx.emit(OneKeyPanelEvent::ItemSelected {
            credential,
    ...
    fn render_present_results(
        ...
        query_result_renderers: &[QueryResultRenderer<OneKeySearchItemAction>],
    ) -> Box<dyn Element> {
    ...
    fn render_send_mode_selection(
        ...
        let send_password = Self::render_clickable_row(
            "Send Password Only",
            OneKeyPanelAction::SendPasswordOnly(credential_pw),
            appearance,
        );

        let send_both = Self::render_clickable_row(
            "Send Username + Password",
            OneKeyPanelAction::SendUsernameThenPassword(credential_both),
            appearance,
        );
...
impl Entity for OneKeyPanel {
    type Event = OneKeyPanelEvent;
}

impl View for OneKeyPanel {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        match &self.mode {
            PanelMode::Searching => {
                ...
                .on_dismiss(|ctx, _app| {
                    ctx.dispatch_typed_action(OneKeyPanelAction::Close);
                })
                .finish()
            }
...
impl TypedActionView for OneKeyPanel {
    type Action = OneKeyPanelAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            OneKeyPanelAction::Close => self.close(ctx),
            OneKeyPanelAction::ResultClicked { result_action, .. } => {
                self.handle_result_selected(*result_action.clone(), ctx)
            }
            OneKeyPanelAction::SendPasswordOnly(credential) => {
                self.handle_send_password_only(credential.clone(), ctx);
            }
            OneKeyPanelAction::SendUsernameThenPassword(credential) => {
